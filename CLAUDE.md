...
**`docs/progress.md`** — บอกว่าทำอะไรไปแล้ว ตอนนี้อยู่ตรงไหน ขั้นถัดไปคืออะไร และกับดักที่เจอมาแล้ว
อย่าเดาสถานะโปรเจคจากโค้ด ให้อ่านไฟล์นั้น

**และเมื่อทำงานชิ้นใดเสร็จ ต้องอัปเดต `docs/progress.md` ก่อนจบงาน** —
เพิ่มสิ่งที่ทำ, ย้ายขั้นถัดไป, บันทึกกับดักใหม่ที่เจอ, อัปเดตวันที่บนหัวไฟล์
งานที่เสร็จแล้วแต่ไม่ได้บันทึก = session ถัดไปต้องมานั่งสืบใหม่
...
สเปคเต็ม: `docs/pos-prd.md` (FR-0…FR-6, BR-1…BR-6, NFR-1…NFR-7)
...
- **`docs/progress.md` — สถานะปัจจุบัน + ขั้นถัดไป (อ่านก่อนเริ่ม อัปเดตก่อนจบ)**
- `docs/design-system.md` — token, กฎ, ตาราง migration, สถานะพอร์ตทั้ง 28 หน้า
- `supabase/README.md` — migration, คำสั่ง, หลักที่ต้องรักษาเรื่อง RLS
