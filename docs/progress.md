...
| ส่วน | สถานะ |
|---|---|
| **หน้าตา** (design system + หน้าจอ) | ✅ **12 หน้าใช้งานได้** — auth 3, ตั้งค่าร้าน, ขาย, ใบเสร็จ, บิล public, **สินค้า 3 หน้า**, `/admin` 2 |
...
### 1. ต้นทุน (FR-5) ← **งานหลักถัดไป**
mockup: `mobile_11`/`tablet_3` (list) + `mobile_2`/`tablet_5` (บันทึกการซื้อ)
DB พร้อมแล้ว — `create_purchase()` เขียน purchase + items ในทรานแซกชันเดียวเหมือน `create_order`
เป็นครึ่งที่ขาดไปของ "กำไร" ตอนนี้แอปรู้แค่รายรับ

### 2. สรุป/รายงาน (FR-6)
`dashboard_mobile`/`dashboard_tablet` + `mobile_7`/`tablet_9` (กำไรรายเดือน) + `mobile_3` (รายการบิล)
ต้องมาหลังต้นทุน ไม่งั้นหน้ากำไรโชว์ได้แค่ครึ่งเดียว

### 3. หน้าจอที่เหลืออีก 17 หน้า
