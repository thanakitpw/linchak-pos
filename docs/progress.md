...
| ส่วน | สถานะ |
|---|---|
| **หน้าตา** (design system + หน้าจอ) | ✅ **9 หน้าใช้งานได้** — auth 3 หน้า, ตั้งค่าร้าน, ขาย, ใบเสร็จ, **บิล public**, `/admin` 2 หน้า |
...
### 1. หน้าสินค้า (FR-2.4) ← **งานหลักถัดไป**
mockup: `mobile_8`/`tablet_6` (list) + `mobile_10`/`tablet_4` (เพิ่ม)
เร่งด่วนกว่าเพื่อน — ตอนนี้เพิ่มสินค้าได้ทางเดียวคือ instant add จากหน้าขาย
ซึ่ง **แก้ราคาหรือลบทีหลังไม่ได้เลย** พิมพ์ราคาผิดครั้งเดียวคือติดอยู่อย่างนั้น
ต้องทำ CRUD หมวดหมู่ (FR-2.1) มาด้วยเพราะ chip กรองในหน้าขายยังว่างอยู่

### 2. ต้นทุน (FR-5) + สรุป (FR-6)
สองแท็บที่เหลือ ตอนนี้เป็น `<NotPortedYet>` stub · mockup มีครบ

### 3. หน้าจอที่เหลืออีก 20 หน้า
