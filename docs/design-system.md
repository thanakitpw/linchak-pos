...
| `mobile_3` / `tablet_8` | รายการบิล | FR-6.4 | ✓ | ✅ `/reports/orders` |
...
| `mobile_11` / `tablet_3` | ต้นทุน (list) | FR-5.1 | ✓ | ✅ `/costs` |
| `mobile_2` / `tablet_5` | บันทึกการซื้อ | FR-5.2/5.3 | ✓ | ✅ `/costs/new` + `/costs/[id]` (แก้ไข — ออกแบบใหม่) |
| `dashboard_mobile` / `dashboard_tablet` | สรุป | FR-6.1/6.2 | ✓ | ✅ `/reports` |
| `mobile_7` / `tablet_9` | กำไรรายเดือน | FR-6.3 | ✓ | ✅ `/reports/profit` |
| `style_guide_*` × 2 | style guide | — | — | ไม่พอร์ต (เอกสารนี้แทน) |

### หน้าที่ PRD ต้องการแต่ไม่มี mockup — ต้องออกแบบใหม่

- ~~หน้าตั้งรหัสผ่านใหม่หลังกดลิงก์จากอีเมล~~ → ✅ `/reset-password/new`
...
- FR-0.4 หน้า trial หมดอายุ / จ่ายเงิน (ตอนนี้มีแค่แถบเตือนในหน้าขาย)
- ~~หน้าแก้ไข (mockup มีแต่ "เพิ่ม")~~ → ✅ ทั้งสินค้าและการซื้อ
- ~~FR-6.2 หน้ารายงาน Daily / Monthly / Custom range~~ → ✅ `/reports/sales` **หน้าเดียวมีทั้งสามโหมด** (ปุ่มลัด + ช่วงวันที่) เพราะทั้งสามคือ "ช่วงวันที่" อันเดียวกัน
