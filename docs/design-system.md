...
| `mobile_4` / `tablet_7` | สมัครใช้งาน | FR-0.1/0.2 | ✓ | ☐ |
| `mobile_1` / `tablet_2` | เข้าสู่ระบบ | FR-0.1 | ✓ | ✅ `/login` (UI เท่านั้น ยังไม่ต่อ auth) |
| `mobile_6` / `tablet_1` | รีเซ็ตรหัสผ่าน | FR-0.1 | ✓ | ☐ |
...
- **FR-1 หน้าตั้งค่าทั้งหมด** (ชื่อร้าน/โลโก้/สาขา/เบอร์, เลข PromptPay, VAT toggle + อัตรา, เลือกภาษา) — ⚙️ มีทุกหน้าแต่ไม่มีปลายทาง **นี่คือช่องว่างใหญ่สุด: ไม่ตั้ง PromptPay = ฟีเจอร์หลักใช้ไม่ได้**
