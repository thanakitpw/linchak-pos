...
Supabase มี key 4 แบบ แต่จริงๆ คือ 2 บทบาท × 2 รุ่น

| บทบาท | รุ่นใหม่ | รุ่นเก่า (เลิกใช้สิ้นปี 2026) | เราใช้ |
|---|---|---|---|
| เปิดเผยได้ ใช้ในเบราว์เซอร์ | `sb_publishable_...` | `anon` (JWT) | ✅ ตัวใหม่ |
| ข้าม RLS ใช้ฝั่ง server | `sb_secret_...` | `service_role` (JWT) | ❌ ไม่ใช้ |

ทั้งสองรุ่นทำงานพร้อมกันได้ แต่ Supabase ประกาศเลิกใช้รุ่นเก่าสิ้นปี 2026 — โปรเจคใหม่ไม่มีเหตุผลจะเริ่มด้วยของที่กำลังจะตาย

### ทำไม `/admin` ไม่ใช้ secret key

ตอนแรกดูเหมือนหลังบ้านต้อง bypass RLS เพื่ออ่านข้อมูลข้ามร้าน — **ไม่ต้อง** เขียน policy ให้ platform admin ผ่านได้แทน:

```sql
create policy "platform admin อ่านได้ทุกร้าน" on workspaces
for select to authenticated
using ( is_member_of(id) or is_platform_admin() );
```

ดีกว่า secret key 3 อย่าง:

1. **มี identity จริง** — admin ล็อกอินด้วยบัญชีตัวเอง `audit_logs.actor_user_id` จึงบันทึกได้ว่าใครทำ · secret key ทำให้ทุก query เป็น "พระเจ้า" ที่ไม่มีชื่อคนทำ ซึ่งขัดกับกฎ 33 ที่ว่าทุก mutation ต้องมี audit
2. **บั๊กไม่รั่วทั้งระบบ** — policy ผิดหนึ่งตารางกระทบตารางเดียว · secret key หลุดครั้งเดียว = ข้อมูลลูกค้าทุกร้าน
3. **ตรวจสอบได้** — policy อยู่ใน migration อ่านและเขียน test ได้ · secret key เป็นสวิตช์ที่มองไม่เห็นในโค้ด

**จะจำเป็นก็ต่อเมื่อ** มีงานที่ไม่มี user session เลย: cron ตัดรอบรายเดือน, webhook จาก payment gateway (เฟสหลัง), Auth Admin API (ลบ user)
ถึงตอนนั้นค่อยเพิ่ม และให้อยู่เฉพาะใน Edge Function หรือ server action ที่แยกชัด
